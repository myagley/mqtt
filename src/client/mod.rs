				...
				ClientState::Up {
					...
					match client_poll(
						...
						*keep_alive,
						...
						Ok(futures::Async::Ready(publication)) => return Ok(futures::Async::Ready(Some(Event::Publication(publication)))),
						Ok(futures::Async::NotReady) => return Ok(futures::Async::NotReady),
						Err(err) =>
...
) -> futures::Poll<crate::ReceivedPublication, Error>
where
	S: tokio::io::AsyncRead + tokio::io::AsyncWrite,
{
	loop {
		...
		if let Some(publication_received) = publication_received {
			return Ok(futures::Async::Ready(publication_received));
		}
